...
async function testSolidServices() {
  ...
  try {
    ...
    console.log(
      `   - Service type: ${practiceAggregateService.constructor.name}`,
    );
    ...
    console.log(
      `   - Service type: ${practiceInstructionService.constructor.name}`,
    );
    ...
    try {
      const practices = await practiceAggregateService.getPractices({
        limit: 5,
      });
      console.log('✅ GET /api/practices endpoint working');
      console.log(
        `   - Response type: ${Array.isArray(practices) ? 'Array' : 'Object'}`,
      );
    ...
    try {
      ...
      console.log(
        `   - Response type: ${Array.isArray(lessons) ? 'Array' : 'Object'}`,
      );
      if ('data' in lessons) {
