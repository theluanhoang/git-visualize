...
describe('OAuthController', () => {
  ...
  it('googleAuth should validate config', async () => {
    ...
    expect(oauthService.validateOAuthConfiguration).toHaveBeenCalledWith(
      'google',
    );
  });

  it('googleAuth propagates config error', async () => {
    oauthService.validateOAuthConfiguration.mockRejectedValue(
      new Error('not configured'),
    );
    await expect(controller.googleAuth({} as any)).rejects.toThrow(
      'not configured',
    );
  });
  ...
  it('google callback should redirect with built url', async () => {
    oauthService.validateOAuthUser = jest
      .fn()
      .mockResolvedValue({ tokens: {} });
    oauthService.buildOAuthRedirectUrl = jest
      .fn()
      .mockResolvedValue('https://redirect');
    const res = { redirect: jest.fn() } as any;
    const req: any = {
      user: { id: 'u1', locale: 'vi' },
      get: () => 'UA',
      ip: '1.1.1.1',
    };
  ...
  it('google callback falls back locale to en', async () => {
    ...
    oauthService.buildOAuthRedirectUrl = jest
      .fn()
      .mockResolvedValue('https://redir');
    const res = { redirect: jest.fn() } as any;
    const req: any = { user: { id: 'u1' }, get: () => 'UA', ip: '1.1.1.1' };
    await controller.googleAuthCallback(req, res);
    expect(oauthService.buildOAuthRedirectUrl).toHaveBeenCalledWith(
      { tokens: {} },
      'en',
    );
  ...
  it('callback propagates service errors', async () => {
    ...
    const res = { redirect: jest.fn() } as any;
    const req: any = { user: { id: 'u1' }, get: () => 'UA', ip: '1.1.1.1' };
    await expect(controller.githubAuthCallback(req, res)).rejects.toThrow(
      'oauth failed',
    );
  });
  ...
  it('github callback should redirect', async () => {
    ...
    oauthService.buildOAuthRedirectUrl = jest
      .fn()
      .mockResolvedValue('https://redirect2');
    const res = { redirect: jest.fn() } as any;
  ...
  it('facebook callback should redirect', async () => {
    ...
    oauthService.buildOAuthRedirectUrl = jest
      .fn()
      .mockResolvedValue('https://redirect3');
    const res = { redirect: jest.fn() } as any;
  ...
  it('unlink should call service with user id and provider', async () => {
    oauthService.unlinkOAuthProviderWithValidation.mockResolvedValue({
      ok: true,
    } as any);
    const result = await controller.unlinkProvider(
      { user: { sub: 'u1' } } as any,
      'google',
    );
    expect(result).toEqual({ ok: true });
    expect(oauthService.unlinkOAuthProviderWithValidation).toHaveBeenCalledWith(
      'u1',
      'google',
    );
  });
  ...
  it('unlink propagates service error', async () => {
    oauthService.unlinkOAuthProviderWithValidation.mockRejectedValue(
      new Error('unknown provider'),
    );
    await expect(
      controller.unlinkProvider({ user: { sub: 'u1' } } as any, 'twitter'),
