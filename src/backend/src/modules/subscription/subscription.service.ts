...
import { Subscription, ESubscriptionStatus, ESubscriptionPlanType } from './subscription.entity';
...
export class SubscriptionService {
    ...
    async createSubscription(userId: string, createDto: CreateSubscriptionDto): Promise<Subscription> {
        try {
            ...
            const subscription = this.subscriptionRepository.create({
                ...
                status: ESubscriptionStatus.ACTIVE, // Activate immediately
                startDate,
                endDate,
            ...
            try {
                await this.updateUserSubscriptionStatus(userId, EUserSubscriptionStatus.PRO, endDate);
            } catch (error) {
                console.error('Error updating user subscription status:', error);
                // If user update fails, we should still return the subscription
                // but log the error
                throw new BadRequestException(`Subscription created but failed to update user status: ${error.message}`);
            }
    ...
    async activateSubscription(subscriptionId: string): Promise<Subscription> {
        ...
        await this.updateUserSubscriptionStatus(subscription.userId, EUserSubscriptionStatus.PRO, subscription.endDate);
    ...
    async renewSubscription(subscriptionId: string, userId: string): Promise<Subscription> {
        const subscription = await this.subscriptionRepository.findOne({
            where: { id: subscriptionId, userId },
        });
        ...
        await this.updateUserSubscriptionStatus(userId, EUserSubscriptionStatus.PRO, updated.endDate);
    ...
    async extendSubscription(subscriptionId: string, days: number): Promise<Subscription> {
        ...
        const updated = await this.subscriptionRepository.save(subscription);
        await this.updateUserSubscriptionStatus(subscription.userId, EUserSubscriptionStatus.PRO, updated.endDate);
    ...
    private async updateUserSubscriptionStatus(
        ...
        try {
            ...
            if (result.affected === 0) {
                throw new NotFoundException(`Failed to update user subscription status for user ${userId}`);
            }
        } catch (error) {
    ...
    // Admin methods
    async getAllSubscriptions(skip?: number, take?: number): Promise<[Subscription[], number]> {
