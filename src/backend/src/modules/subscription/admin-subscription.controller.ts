...
export class AdminSubscriptionController {
    ...
    async getAllSubscriptions(@Query() query: AdminSubscriptionQueryDto) {
        ...
        const [subscriptions, total] = await this.subscriptionService.getAllSubscriptions(skip, take);
    ...
    async getSubscriptionStats() {
        const [allSubscriptions] = await this.subscriptionService.getAllSubscriptions(0, 10000);
        
        const stats = {
            total: allSubscriptions.length,
            active: allSubscriptions.filter((s) => s.status === ESubscriptionStatus.ACTIVE).length,
            cancelled: allSubscriptions.filter((s) => s.status === ESubscriptionStatus.CANCELLED).length,
            expired: allSubscriptions.filter((s) => s.status === ESubscriptionStatus.EXPIRED).length,
            pending: allSubscriptions.filter((s) => s.status === ESubscriptionStatus.PENDING).length,
            monthly: allSubscriptions.filter((s) => s.planType === ESubscriptionPlanType.MONTHLY).length,
            yearly: allSubscriptions.filter((s) => s.planType === ESubscriptionPlanType.YEARLY).length,
        };
    ...
    async getProUsers(@Query() query: ProUsersQueryDto) {
        ...
        const [subscriptions] = await this.subscriptionService.getAllSubscriptions(skip, take);
        const activeSubscriptions = subscriptions.filter((s) => s.status === ESubscriptionStatus.ACTIVE);
    ...
    async activateSubscription(@Param('id') id: string): Promise<SubscriptionResponseDto> {
    ...
    @ApiResponse({ status: 200, description: 'Subscription deactivated', type: SubscriptionResponseDto })
    async deactivateSubscription(@Param('id') id: string): Promise<SubscriptionResponseDto> {
        const subscription = await this.subscriptionService.deactivateSubscription(id);
        return this.mapToResponse(subscription);
    }
    ...
    @ApiResponse({ status: 200, description: 'Subscription extended', type: SubscriptionResponseDto })
    async extendSubscription(
        @Param('id') id: string,
        @Body() dto: ExtendSubscriptionDto,
    ): Promise<SubscriptionResponseDto> {
        const subscription = await this.subscriptionService.extendSubscription(id, dto.days);
        return this.mapToResponse(subscription);
    }
    ...
    async cancelSubscription(@Param('id') id: string): Promise<SubscriptionResponseDto> {
        const subscription = await this.subscriptionService.getSubscriptionById(id);
        const cancelled = await this.subscriptionService.cancelSubscription(id, subscription.userId);
