...
export class QuizAggregateService {
  ...
  async getQuizzes(
    query: GetQuizzesQueryDto,
  ): Promise<
    Quiz | { data: Quiz[]; total: number; limit: number; offset: number }
  > {
  ...
  async createQuiz(createQuizDTO: CreateQuizDTO): Promise<Quiz> {
    ...
    try {
      ...
      const savedQuiz = await this.quizEntityService.create(
        createQuizDTO,
        queryRunner,
      );
  ...
  private async createRelatedEntities(
    ...
    if (dto.questions?.length) {
      const questions = await this.quizQuestionService.createMany(
        quizId,
        dto.questions,
        queryRunner,
      );
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (questionDTO.options?.length) {
          await this.quizOptionService.createMany(
            question.id,
            questionDTO.options,
            queryRunner,
          );
