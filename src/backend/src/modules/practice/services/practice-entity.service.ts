...
export class PracticeEntityService {
  ...
  async create(
    ...
    const practiceRepo = queryRunner
      ? queryRunner.manager.getRepository(Practice)
    ...
    const practice = practiceRepo.create({
      ...
      isActive:
        createPracticeDTO.isActive !== undefined
          ? createPracticeDTO.isActive
          : true,
  ...
  async getAggregateStats(): Promise<{
    ...
    const [totalTimeSpent, completionStats] = await Promise.all([
      this.practiceRepository
        ...
        .select(
          'SUM(practice.estimatedTime * practice.completions)',
          'totalMinutes',
        )
    ...
    return {
      totalTimeSpent: {
        totalMinutes: totalTimeSpent?.totalMinutes
          ? parseFloat(totalTimeSpent.totalMinutes)
          : 0,
      },
      completionStats: {
        totalCompletions: completionStats?.totalCompletions
          ? parseFloat(completionStats.totalCompletions)
          : 0,
        totalViews: completionStats?.totalViews
          ? parseFloat(completionStats.totalViews)
          : 0,
      },
    };
  }
