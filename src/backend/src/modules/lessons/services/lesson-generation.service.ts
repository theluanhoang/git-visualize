...
import {
  ...
  GenerationResult,
} from './gemini-generation.service';
import {
  GenerateLessonDto,
  Language,
  ModelType,
      ...
      switch (dto.sourceType) {
        case SourceType.URL:
          if (!dto.url) {
            throw new BadRequestException(
              'URL is required when sourceType is url',
            );
          }
          extractedContent = await this.contentExtractionService.extractFromUrl(
            dto.url,
          );
          break;
        ...
        case SourceType.FILE:
          if (!dto.fileBuffer || !dto.fileName) {
            throw new BadRequestException(
              'fileBuffer and fileName are required when sourceType is file',
            );
          }
          if (dto.fileName.toLowerCase().endsWith('.pdf')) {
            extractedContent =
              await this.contentExtractionService.extractFromPdf(
                dto.fileBuffer,
              );
          } else if (dto.fileName.toLowerCase().endsWith('.docx')) {
            extractedContent =
              await this.contentExtractionService.extractFromDocx(
                dto.fileBuffer,
              );
          } else {
            throw new BadRequestException(
