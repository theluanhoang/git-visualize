...
import { LessonWithPractices, GetLessonsResponse } from './types';
...
export class LessonService {
  ...
  async getLessonsAggregateStats(): Promise<{
    totalLessons: number;
    totalViews: number;
  }> {
  ...
  async getLessons(
    ...
    if (q) {
      baseQb.andWhere(
        '(lesson.title ILIKE :q OR lesson.description ILIKE :q)',
        { q: `%${q}%` },
      );
    }
    ...
    if (isAdmin) {
      ...
      baseQb.andWhere(
        '(lesson.isPublic = :isPublic OR lesson.authorId IS NULL)',
        {
          isPublic: true,
        },
      );
      ...
      if (userId) {
        ...
        if (slug) {
          ...
          baseQb.andWhere(
            '(lesson.authorId = :userId OR (lesson.isPublic = :isPublic AND lesson.authorId IS NULL))',
            {
              userId,
              isPublic: true,
            },
          ...
          baseQb.andWhere(
            '(lesson.isPublic = :isPublic OR lesson.authorId = :userId)',
            {
              isPublic: true,
              userId,
            },
    ...
    const [completionCounts] = await Promise.all([
      lessonIds.length > 0
        ? this.getPracticeCompletionCountsForLessons(lessonIds)
        : Promise.resolve({}),
    ]);
    ...
    if (includePractices) {
      ...
      return { data, total, limit, offset } as GetLessonsResponse<
        LessonWithPractices & {
    ...
    return {
      data: lessonsWithData,
      total,
      limit,
      offset,
    } as GetLessonsResponse<
      Lesson & { averageRating: number; completedUsersCount: number }
    >;
  }

  private async fetchPracticesForLessons(
    lessons: (Lesson & { averageRating?: number })[],
  ): Promise<(LessonWithPractices & { averageRating?: number })[]> {
    return Promise.all(
      lessons.map(async (lesson) => {
        ...
        const practicesResult =
          await this.practiceAggregateService.getPractices({
            lessonSlug: lesson.slug,
            includeRelations: true,
            publishedOnly: false, // Include practices for draft lessons
          });
        const practices =
          'data' in practicesResult
            ? practicesResult.data
            : [practicesResult as any];
        return {
          ...lesson,
          practices: practices,
  ...
  async createLesson(
    ...
    if (existingLesson) {
      throw new ConflictException(
        `Slug "${createLessonDTO.slug}" already exists for this user`,
      );
    }
  ...
  async updateLesson(
    ...
    if (dto.slug && dto.slug !== existing.slug) {
      ...
      if (conflictingLesson && conflictingLesson.id !== id) {
        throw new ConflictException(
          `Slug "${dto.slug}" already exists for this user`,
        );
      }
    }
  ...
  async getMyLessons(
    ...
    limit?: number,
    offset?: number,
  ): Promise<GetLessonsResponse<Lesson>> {
  ...
  async updateLessonViews(lessonId: string, views: number): Promise<void> {
    await this.lessonRepository.update({ id: lessonId }, { views });
  }
  ...
  async getPracticeCompletionCount(lessonId: string): Promise<number> {
    ...
    const mappings =
      await this.practiceRepoStateService.getPracticeUserMappings(practiceIds);
    const uniqueUsers = new Set(mappings.map((m) => m.userId));

    return uniqueUsers.size;
  }

  async getPracticeCompletionCountsForLessons(
    lessonIds: string[],
  ): Promise<Record<string, number>> {
    return this.practiceRepoStateService.getCompletionCountsByLessons(
      lessonIds,
    );
  }
}
