...
export class LessonController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Create a new lesson (Admin or Pro subscription required)',
  })
  ...
  async deleteLesson(@UserId() userId: string, @Param('id') id: string) {
  ...
  @ApiOkResponse({
    ...
    type: GenerateLessonResponseDto,
  })
    ...
    if (sourceType === SourceType.FILE) {
      if (!file) {
        throw new BadRequestException(
          'File is required when sourceType is file',
        );
      }
  ...
  @ApiOkResponse({
    description: 'Lesson view statistics retrieved successfully',
  })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({
    summary: 'Check if authenticated user has viewed a specific lesson',
  })
  ...
  async hasUserViewedLesson(@UserId() userId: string, @Param('id') id: string) {
    const hasViewed = await this.lessonViewService.hasUserViewedLesson(
      userId,
      id,
    );
    const viewCount = await this.lessonViewService.getUserLessonViewCount(
      userId,
      id,
    );
    return {
  ...
  @ApiBadRequestResponse({
    description: 'Invalid rating value or lesson not found',
  })
  async createRating(
    @UserId() userId: string,
    ...
    try {
      ...
      const ratingWithUser = await this.ratingService.getUserRating(
        userId,
        lessonId,
      );
  ...
  @ApiBadRequestResponse({
    description: 'Invalid rating value or rating not found',
  })
  async updateRating(
    @UserId() userId: string,
    @Param('id') lessonId: string,
  ...
  @ApiOkResponse({
    ...
    type: LessonRatingStatsDto,
  })
  async getLessonRatingStats(
    @Param('id') lessonId: string,
  ): Promise<LessonRatingStatsDto> {
