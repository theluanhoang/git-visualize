...
export class GitEngineController {
  ...
  executeCommand(
    @Body() request: GitCommandRequest,
  ): GitCommandResponse | null {
    const result = this.gitEngineService.executeCommandWithState(
  ...
  @ApiOperation({
    summary:
      'Validate user repositoryState against the practice goalRepositoryState',
  })
  @ApiBody({ type: PracticeValidationDto })
  @ApiResponse({ status: 200, description: 'Validation result returned' })
  async validatePractice(
    @Body() request: PracticeValidationDto,
  ): Promise<PracticeValidationResponse> {
    const userState =
      request.userRepositoryState as unknown as IRepositoryState;
    const result = await this.gitEngineService.validatePractice(
      request.practiceId,
      userState,
    );
