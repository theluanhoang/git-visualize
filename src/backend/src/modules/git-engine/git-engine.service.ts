...
export class GitEngineService {
  ...
  executeCommandWithState(
    state: IRepositoryState | null | undefined,
    command: string,
  ): GitCommandResponse | null {
  ...
  executeCommand(command: string): GitCommandResponse | null {
    ...
    if (!cmd) {
      return this.response(
        "git: no command provided. See 'git --help'.",
        false,
      );
    }
  ...
  private commit(args: string[]): GitCommandResponse {
    ...
    const branch = this.repositoryState.branches.find(
      (b) => b.name === branchName,
    );

    if (!branch) {
      return this.response(
        `fatal: current branch '${branchName}' not found`,
        false,
      );
    }

    const allowEmpty =
      args.includes('--allow-empty') || args.includes('--allow-empty-message');

    const stagingArea = this.repositoryState.stagingArea || [];
    const workingDir = this.repositoryState.workingDirectory || [];

    if (!allowEmpty && stagingArea.length === 0) {
      const hasChanges = workingDir.some(
        (f) =>
          f.status === FileStatus.MODIFIED ||
          f.status === FileStatus.UNTRACKED ||
      ...
      if (!hasChanges) {
        return this.response('nothing to commit, working tree clean', false);
      }
    ...
    if (messageIndex === -1 || !args[messageIndex + 1]) {
      return this.response(
        'error: commit message not provided (use -m "msg")',
        false,
      );
    }

    const firstToken = args[messageIndex + 1];
    const quoteChar = firstToken.startsWith('"')
      ? '"'
    ...
    if (!quoteChar) {
      return this.response(
        'error: commit message must be quoted (use -m "your message")',
        false,
      );
    }
    ...
    if (
      message.length >= 2 &&
      message.startsWith(quoteChar) &&
      message.endsWith(quoteChar)
    ) {
    ...
    if (isAmend && this.repositoryState.commits.length > 0) {
      const lastCommit =
        this.repositoryState.commits[this.repositoryState.commits.length - 1];
  ...
  private branch(args: string[]): GitCommandResponse {
    ...
    const exists = this.repositoryState.branches.some(
      (b) => b.name === newBranchName,
    );
    if (exists) {
      return this.response(
        `fatal: A branch named '${newBranchName}' already exists.`,
        false,
      );
    }

    const currentBranch = this.repositoryState.branches.find(
      (b) => b.name === head.ref,
    );
  ...
  private switch(args: string[]): GitCommandResponse {
    ...
    const existingBranch = this.repositoryState.branches.find(
      (b) => b.name === target,
    );
  ...
  async validatePractice(
    practiceId: string,
    userRepositoryState: IRepositoryState,
    ...
    try {
      ...
      const differences = this.compareRepositoryStates(
        goalState,
        userRepositoryState,
      );
      ...
      if (isCorrect) {
        ...
        message =
          'Congratulations! You have successfully completed the practice.';
      } else {
        feedback = `Found ${differences.length} difference(s) between your repository state and the goal.`;
        message =
          'Your repository state is close but not exactly matching the goal.';
      }
  ...
  private compareRepositoryStates(
    goalState: IRepositoryState,
    userState: IRepositoryState,
  ): RepositoryDifference[] {
  ...
  private calculateScore(
    ...
    for (const category of categories) {
      const categoryDifferences = differences.filter(
        (d) => d.type === category,
      );
      penalty += Math.min(
        categoryDifferences.length * penaltyPerDifference,
        50,
      );
    }
