...
describe('GitEngineController', () => {
  ...
  it('executeCommand forwards to service', () => {
    service.executeCommandWithState.mockReturnValue({
      success: true,
      output: 'ok',
      repositoryState: {},
    } as any);
    const res = controller.executeCommand({
      command: 'git init',
      repositoryState: {},
    } as any);
    expect(res).toEqual({ success: true, output: 'ok', repositoryState: {} });
    expect(service.executeCommandWithState).toHaveBeenCalledWith(
      {},
      'git init',
    );
  });
  ...
  it('executeCommand propagates service errors', () => {
    service.executeCommandWithState.mockImplementation(() => {
  ...
  it('validatePractice returns detailed diff when invalid', async () => {
    service.validatePractice.mockResolvedValue({
      valid: false,
      diffs: [{ path: 'branch', expected: 'main', actual: 'dev' }],
    } as any);
    const res = await controller.validatePractice({
      practiceId: 'p1',
      userRepositoryState: {},
    } as any);
    expect(res).toEqual({
